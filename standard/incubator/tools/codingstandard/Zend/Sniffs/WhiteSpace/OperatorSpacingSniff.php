...
class Zend_Sniffs_WhiteSpace_OperatorSpacingSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] === T_EQUAL) {
            // Skip for '=&' case.
            if (isset($tokens[($stackPtr + 1)]) === true and $tokens[($stackPtr + 1)]['code'] === T_BITWISE_AND) {
                return;
            }
        ...
        if ($tokens[$stackPtr]['code'] === T_BITWISE_AND) {
            // If its not a reference, then we expect one space either side of the bitwise operator
            ...
            if ($tokens[$stackPtr]['code'] === T_MINUS) {
                ...
                if ($tokens[$number]['code'] === T_LNUMBER) {
                    ...
                    if ($tokens[$semi]['code'] === T_SEMICOLON) {
                        if ($prev !== false and (in_array($tokens[$prev]['code'],
