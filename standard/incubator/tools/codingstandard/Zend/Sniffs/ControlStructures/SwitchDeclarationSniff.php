...
class Zend_Sniffs_ControlStructures_SwitchDeclarationSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        while ($nextCase !== false) {
            ...
            if ($tokens[($nextCase + 1)]['type'] !== 'T_WHITESPACE' or $tokens[($nextCase + 1)]['content'] !== ' ') {
                $error = 'CASE keyword must be followed by a single space';
                $phpcsFile->addError($error, $nextCase);
            }

            if (array_key_exists('scope_opener', $tokens[$nextCase]) === true) {
            ...
            if ($nextBreak !== false and isset($tokens[$nextBreak]['scope_condition']) === true) {
                ...
                if ($tokens[$nextBreak]['scope_condition'] === $nextCase) {
                    ...
                    // Ensure empty CASE statements are not allowed. They must have some code content in them
                    // or a defined comment "// break intentionally omitted"
                    $foundContent = false;
                    $old          = 0;
                    for ($i = ($tokens[$nextCase]['scope_opener'] + 1); $i < $nextBreak; $i++) {
                        if ($tokens[$i]['code'] === T_CASE) {
                            ...
                            if ($tokens[$old]['code'] === T_CASE) {
                                $next = $phpcsFile->findNext(array(T_COMMENT), ($nextCase + 1),
                                                             $switch['scope_closer']);
                    ...
                    // Ensure there is no blank line before the BREAK statement
                    ...
                    // Ensure the BREAK statement is followed by a single blank line, or the end switch brace
                    $breakLine = $tokens[$nextBreak]['line'];
                    ...
                    if ($nextLine !== ($breakLine + 2) and $i !== $tokens[$stackPtr]['scope_closer']) {
                        $error = 'BREAK statements must be followed by a single blank line';
                        $phpcsFile->addError($error, $nextBreak);
                    }
                }
            } else {
                if (array_key_exists('scope_closer', $tokens[$nextCase]) === true) {
                    $nextBreak = $tokens[$nextCase]['scope_closer'];
                }
            }

            // Ensure CASE statements are not followed by blank lines
        ...
        if ($default !== false) {
            ...
            // Ensure empty DEFAULT statements are not allowed. They must (at least) have a comment
            // describing why the default case is being ignored
            $foundContent = false;
            ...
            // Ensure DEFAULT statements are not followed by blank lines
