...
class Zend_Sniffs_Functions_FunctionCallArgumentSpacingSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        // Skip tokens that are the names of functions or classes within their definitions. For example:
        // function myFunction...
        ...
        if ($tokens[$functionKeyword]['code'] === T_FUNCTION or $tokens[$functionKeyword]['code'] === T_CLASS) {
            return;
        }
        ...
        $nextSeperator = $phpcsFile->findNext(array(T_COMMA, T_VARIABLE), ($nextSeperator + 1), $closeBracket);
        while ($nextSeperator !== false) {
            ...
            if ($tokens[$nextSeperator]['code'] === T_COMMA) {
                ...
                // Token is a variable
                $nextToken = $phpcsFile->findNext(PHP_CodeSniffer_Tokens::$emptyTokens, ($nextSeperator + 1),
                                                  $closeBracket, true);
                if ($nextToken !== false) {
