 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class Zend_Db_Table_Abstract implements SplSubject
{
    ...
    public function __construct($config = array())
    {
        ...
        if ($this->notify('preInit', $this) === false) {
            return;
        }
        $this->init();
        $this->notify('postInit', $this);
    }
     ...
     */
    public function addPlugin($plugin, $className = Zend_Db_Table_Plugin_Broker::ALL,
                              $position = Zend_Db_Table_Plugin_Broker::APPEND,
                              $positionKey = null)
    {
        Zend_Db_Table_Plugin_Broker::registerPlugin($plugin, get_class($this), $position, $positionKey);
    }
    ...
    {
        Zend_Db_Table_Plugin_Broker::registerPlugin($observer);
        return $this;
    }
    ...
    public function update(array $data, $where)
    {
         ...
         */
        if ($this->notify('preUpdate', $this, $data, $where) === false) {
            return 0;
        }
         ...
         */
        $this->notify('postUpdate', $this, $data, $where);
    ...
    {
        $this->notify('preFetch', $this, $select);
        $stmt = $this->_db->query($select);
        $data = $stmt->fetchAll(Zend_Db::FETCH_ASSOC);
        $this->notify('postFetch', $this, $data);
        return $data;
    }
