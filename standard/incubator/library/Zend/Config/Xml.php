...
class Zend_Config_Xml extends Zend_Config
{
    ...
    public function __construct($filename, $section = null, $allowModifications = false)
    {
        ...
        } else if (is_array($section) === true) {
            $dataArray = array();
            foreach ($section as $sectionName) {
                if (isset($config->$sectionName) === false) {
                    require_once 'Zend/Config/Exception.php';
    ...
    {
        if (isset($element->$section) === false) {
            require_once 'Zend/Config/Exception.php';
        ...
        if (isset($thisSection['extends']) === true) {
    ...
    protected function _toArray(SimpleXMLElement $xmlObject)
    {
        ...
        if (count($xmlObject->children()) > 0) {
            foreach ($xmlObject->children() as $key => $value) {
                ...
                if (array_key_exists($key, $config) === true) {
                    if (is_array($config[$key]) === false or array_key_exists(0, $config[$key]) === false) {
                        $config[$key] = array($config[$key]);
                    }
        ...
        } else if (isset($xmlObject['extends']) === false and count($config) === 0) {
    ...
    {
        if (is_array($firstArray) === true and is_array($secondArray) === true) {
            foreach ($secondArray as $key => $value) {
                if (isset($firstArray[$key]) === true) {
