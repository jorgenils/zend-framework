...
class ZFModule_Forum
{
    ...
    /////////////////////////////
    // ==> SECTION: l10n <==
    /* Time offset between user and server's time.
     * @var integer  seconds
     */
    protected static $_timeOffset = 0;

    /* List of country names in the native language of the user, per their locale
     * @var array   of country names (UTF8)
     */
    protected static $_countries = null;

    /* Locale of user making this request.
     * @var Zend_Locale
     */
    protected static $_userLocale = null;
    ...
    /**
     * Using a two-letter region code, return the name of the corresponding country,
     * if possible.  Not all region codes map to country names.
     * @param  string  $code  Two-letter country code.
     */
    public static function getCountry($code)
    {
        if (self::$_countries === null) {
            // list of country names in user's language
            self::$_countries = self::$_userLocale->getCountryTranslationList();
            // country code index of current user for array above
        }
