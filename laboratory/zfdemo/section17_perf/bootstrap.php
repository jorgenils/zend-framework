    ...
    public function preDispatch(Zend_Controller_Request_Abstract $request)
    {
        ...
        if (is_readable($moduleInit)) {
            ...
            // ==> SECTION: perf <==
                ...
                // Use Zend_Cache_Frontend_File, which links a cache result to the modification time of a file,
                // so that any change to the file invalidates the cache on the next cache hit.
                $back  = $registry['config']->cache->back->toArray();
                $front = $registry['config']->cache->front->toArray();
                $front['masterFile'] = $modulesIni; // sync cache result to modification time of this file
                $back['cacheDir'] = $registry['temporaryDir']; // . DIRECTORY_SEPARATOR . 'modules';
                $cache = Zend_Cache::factory('File', 'File', $front, $back);

                // if cache miss, then create Zend_Config_Ini object, and save to cache
                if (!($config = $cache->load('modules_ini'))) {
                    $config = new Zend_Config_Ini($modulesIni, $moduleName);
                    $cache->save($config);
                }
            }
