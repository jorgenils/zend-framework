...
require_once 'Zend/Filter/Builder/FilterFactory/Interface.php';

/**
 * Zend Filter Builder Filter Factory
 *
 * Instantiates Filter objects, given only the "base name" of the filter 
 * class. A set of "namespace" class prefixes to search in can be specified in 
 * the options. This class always includes 'Zend_Filter_' at the end of the 
 ...
 *
 * This can be used as a quick way to create filter objects without having to 
 * manually include the class file and instantiate the object. The 
 * Zend_Filter_Builder_FluentFacade (and _FluentAdder) classes also use this 
...
class Zend_Filter_Builder_FilterFactory implements Zend_Filter_Builder_FilterFactory_Interface
{
    ...
    public function setOptions(array $options)
    {
        if (empty($this->_options['namespaces'])) {
            $this->_options['namespaces'] = array('Zend_Filter_');
        }
    ...
    public function create($name, $args)
    {
        ...
        $filter = $valClass->newInstanceArgs($args);
